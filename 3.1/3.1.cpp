...
vector<Edge> min_ostov(vector<vector<int>>& mat, int start) //start - это вершина, поэтому от 0 до максимальной вершины
{
	...
	{
		cout << "Неверно указана вершина старата. Вершина старта устанавливается в ноль." << endl;
		start = 0;
	}
	...
	for (int i = 0; i < mat.size() - 1; i++)//Записывает все вершины в виде списка смежности. (Если 0 1 есть в списке,то 1 0 уже не добавляем)
	{
	...
	sort(list_vert.begin(), list_vert.end(),
		[](Edge& a, Edge& b) //Лямбда - для сортировки по убыванию (потому что удалям с конца)
		{
...
int main()
{
	...
	cout << endl;
	vector<Edge> ostov = min_ostov(mat, 0);
