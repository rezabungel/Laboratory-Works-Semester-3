...
Реализовать функцию подсчета степени (количества инцидентных
ребер) вершин в полученном дереве (обход дерева сделать на основе
поиска в ширину). Реализовать функцию подсчета средней степени по
всему дереву.
...
//Степень вершины - количество рёбер, соединенных с этой вершиной.
vector<int> stepen_min_ostov(vector<Edge>& min_ostov) //Реализовано на основе BFS - Breadth-First Search (поиск в ширину или обход в ширину).
{
	vector<int> used(min_ostov.size() + 1, 0);//Вектор, хранящий информацию о взятых вершинах.

	vector<int> stepen(min_ostov.size() + 1, 0);//Хранит степень всех вершин (сколько рёбер выходят из неё).
...
//Функция подсчета средней степени дерева минимального остова.
double srednyaya_stepen_min_ostov(vector<Edge>& min_ostov)
{
	vector<int> stepen = stepen_min_ostov(min_ostov);
...
vector<int> BFS(vector<vector<int>>& mat, int start) //Обход в ширину, для случая, когда мы передаем матрицу смежности.
{
	if (start < 0 || start >= mat.size())
	{
		cout << "Неверно указана вершина старта. Вершина старта устанавливается в ноль." << endl;
...
vector<int> BFS(vector<Edge>& min_ostov, int start)//Обход в ширину, для случая, когда мы передаем минимальный остов.(В данной реализации возвращает минимальное количество ребер от стартовой вершины, до всех остальных (не смотрим на вес, только на количество)).
{
	...
	while (!graph_node.empty())
	{
		...
		for (int i = 0; i < min_ostov.size(); i++)
		{
			if (min_ostov[i].get_from() == vertex && used[min_ostov[i].get_to()] == 0)
			{
				...
				if (dist[min_ostov[i].get_to()] == -1)
				{
					dist[min_ostov[i].get_to()] = dist[vertex] + 1;
				}
			}
		}
		used[vertex] = 1;//Закончили обработку вершины
...
int main()
{
	...
	cout << "BFS - обход в ширину -передаём матрицу смежности-(вернёт минимальное кол-во рёбер от стартовой вершины (" << start << ") до всех остальных): ";
	print_vector(graph_traversal);

	graph_traversal = BFS(ostov, start);
	cout << "BFS - обход в ширину -передаём минимальный остов-(вернёт минимальное кол-во рёбер от стартовой вершины (" << start << ") до всех остальных): ";
	print_vector(graph_traversal);
	...
	graph_traversal = stepen_min_ostov(ostov);
	cout << "Степень минимального остова: ";
	print_vector(graph_traversal);

	sred_step = srednyaya_stepen_min_ostov(ostov);
	cout << "Средняя степень минимального остова: " << sred_step << endl;
	...
	graph_traversal_1 = stepen_min_ostov(ostov_1);
	cout << "Степень минимального остова: ";
	print_vector(graph_traversal_1);

	sred_step_1 = srednyaya_stepen_min_ostov(ostov_1);
	cout << "Средняя степень минимального остова: " << sred_step_1 << endl;
